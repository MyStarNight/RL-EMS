...
def main():
    ...
    money_without_ess, money_with_ess, money_saved, money_without_ess_list, money_with_ess_list = function.money_saved_calculation(actions, grid_price, grid_consumption, charge_rate=ems.charge_rate)
    ...
    return money_with_ess_list

def visualization(money_with_rl, money_with_threshold, money_without_ess):
    time_list = [i for i in range(len(money_with_rl))]
    plt.plot(time_list, money_with_rl, label='RL',)
    plt.plot(time_list,money_with_threshold, label='Threshold',)
    plt.plot(time_list, money_without_ess, label='Normal',)

    plt.title('Comparison of Different Policies')
    plt.xlabel('Time (h)')
    plt.ylabel('Amount (Eu/1000)')
